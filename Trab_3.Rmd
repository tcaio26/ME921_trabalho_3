...
chars_reduzido = characters_intercations %>% arrange(desc(interactions)) %>% head(100) %>% pull(char)
...
```{r função final}
WSBM = function(X, Y, Q, iterations=50){
  n = nrow(X)
  g = kmeans(X, Q)
  g = g$cluster
  mu = mean(Y[Y>0])
...
crit_WSBM = function(X, Y,range = 1:3, iterations=50){
  ...
  return(list(ICL = icl, BIC = bic, log_lik = v))
}
```
